...
DECLARE
  ...
  v_all_markets text[] := v_cards_markets || v_corners_markets;
BEGIN
  ...
  FOR v_pred IN
    SELECT * FROM public.predictions p
     WHERE p.match_id = p_match_id
       AND p.status IN ('won'::public.prediction_status,'lost'::public.prediction_status)
       AND p.market::text = ANY(v_all_markets)
     FOR UPDATE
  LOOP
    v_won := false;
