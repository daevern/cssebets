...
BEGIN
  ...
  IF v_market.odds > p_max_odds THEN
    RAISE EXCEPTION 'Odds changed';
  END IF;

  SELECT id, starts_at, status
    INTO v_race
    FROM public.f1_races
   WHERE id = v_market.race_id;

  IF v_race.starts_at <= now() THEN
  ...
  VALUES (
    ...
    'f1_bet',
    v_bet_id,
    jsonb_build_object('source', 'f1', 'market_id', p_market_id)
  );
...
BEGIN
  ...
  SELECT id, season, market_type, selection_key, label, odds, status
    INTO v_market
    FROM public.f1_championship_markets
   WHERE id = p_market_id
   FOR UPDATE;
  ...
  INSERT INTO public.f1_championship_bets (
    user_id,
    market_id,
    season,
    market_type,
  ...
  VALUES (
    ...
    'f1_championship_bet',
    v_bet_id,
    jsonb_build_object('source', 'f1_champ', 'market_id', p_market_id)
  );
