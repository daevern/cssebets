CREATE OR REPLACE FUNCTION public.f1_championship_season_locked(p_season integer)
RETURNS boolean
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.f1_races
...
BEGIN
  ...
  IF public.f1_championship_season_locked(v_bet.season) THEN RAISE EXCEPTION 'MATCH_LOCKED'; END IF;
...
BEGIN
  ...
  PERFORM public.accounting_release_liability('f1_championship_bet', v_bet.id, 'USER_CANCEL');
...
REVOKE ALL ON FUNCTION public.f1_championship_season_locked(integer) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.f1_championship_season_locked(integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.edit_f1_championship_bet_stake(uuid,uuid,numeric) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.edit_f1_championship_bet_stake(uuid,uuid,numeric) TO service_role;
REVOKE ALL ON FUNCTION public.cancel_f1_championship_bet(uuid,uuid) FROM PUBLIC, anon, authenticated;
