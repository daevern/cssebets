...
-- 1) Default house margin = 25%
ALTER TABLE public.platform_settings ALTER COLUMN margin_pct SET DEFAULT 25.0;
UPDATE public.platform_settings
   SET margin_pct = 25.0, updated_at = now()
 WHERE id = 1 AND margin_pct < 25.0;

-- 2) Rewrite seeder: 25% house margin, 1.01 floor, simulation-gated placeholders.
...
BEGIN
  ...
  SELECT reference_odds, COALESCE(is_simulation, false)
    INTO v_odds, v_is_sim
    FROM public.matches WHERE id = p_match_id;

  -- Hotfix: live matches must NEVER receive placeholder odds. Only simulation
  -- matches are allowed to seed from the synthetic 2.10/3.30/3.60 fallback.
  ...
  THEN
    IF NOT v_is_sim THEN
      -- Live match: refuse to fabricate. Leave odds empty; betting is blocked
      -- by check_match_market_betting (ODDS_MISSING / ODDS_AWAITING_SYNC).
      RETURN;
    END IF;
  ...
  -- Strip bookmaker overround → fair probabilities.
  v_ph := 1.0 / v_ho; v_pd := 1.0 / v_do; v_pa := 1.0 / v_ao;
  v_t := v_ph + v_pd + v_pa;
  v_ph := v_ph / v_t; v_pd := v_pd / v_t; v_pa := v_pa / v_t;

  -- Fit a Poisson grid to the fair 1X2 probabilities.
  v_mu := 1.8;
  WHILE v_mu <= 3.6 LOOP
    v_s := -1.6;
    WHILE v_s <= 1.6 LOOP
  ...
  -- Correct score: OTHER is the residual probability of all unlisted scores.
  v_listed_sum := 0;
  ...
  -- Apply CSSEBets house margin: house_prob = fair_prob * overround;
  -- display_odds = 1 / house_prob = (1 / fair_prob) / overround.
  -- Floor at 1.01, cap at per-market ceiling.
...
-- 3) Re-seed existing simulation matches so new 25% margin takes effect.
DO $$
DECLARE r record;
BEGIN
  FOR r IN
    SELECT id FROM public.matches
     WHERE status = 'scheduled' AND COALESCE(is_simulation, false) = true
  LOOP
