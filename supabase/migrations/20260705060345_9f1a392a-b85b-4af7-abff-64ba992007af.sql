-- Grade red_card_match and first_card from match_events (which the sync actually populates)
-- instead of matches.red_card_occurred / matches.first_card_team (which the sync never writes).
-- Keep the "don't grade on partial data" guard from the previous fix.
...
BEGIN
  ...
  -- Derive red_card_occurred and first_card_team from match_events (the sync writes there).
  -- Only treat these as authoritative once the match is finished AND at least one event exists
  -- with created_at close to the finish (guard against pre-fulltime partial events).
  SELECT EXISTS (
  ...
  SELECT EXISTS (
    SELECT 1 FROM public.match_events
     WHERE match_id = p_match_id
       AND created_at >= COALESCE(v_m.updated_at, now()) - interval '2 minutes'
      ...
      ELSE
        ...
        CASE
          ...
          WHEN v_pred.market::text = 'red_card_match' THEN
            -- Derive from events; require the events sweep to be fresh.
            -- If no events at all for a finished match, be conservative and skip (leave pending).
          ...
          WHEN v_pred.market::text = 'first_card' THEN
            ...
            IF v_first_card_team IS NULL THEN
              -- No card in the whole match; can't grade first_card either way -> void.
              v_void := true;
            ELSE
