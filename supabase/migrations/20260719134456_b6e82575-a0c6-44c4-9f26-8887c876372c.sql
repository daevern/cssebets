...
-- One-off regrade for a specific real user's UFC bets. On a fresh/CI database
-- this user doesn't exist, so skip the writes below entirely instead of
-- failing on wallet_transactions' FK to auth.users.
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE id = '1a8f9625-5eb2-4d3b-88d1-01b6a055e410'::uuid) THEN
    RETURN;
  END IF;
