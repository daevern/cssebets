CREATE OR REPLACE FUNCTION public.accounting_post_arcade_settlement(
  ...
  p_wallet_category text DEFAULT NULL::text,
  p_wallet_idem text DEFAULT NULL::text
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
  ...
  IF v_stake > 0 THEN
    v_stake_res := public.accounting_post_journal(
      ...
      p_metadata => coalesce(p_meta, '{}'::jsonb) || jsonb_build_object(
        'wallet_category', p_wallet_category,
        'wallet_idem', p_wallet_idem
      ),
