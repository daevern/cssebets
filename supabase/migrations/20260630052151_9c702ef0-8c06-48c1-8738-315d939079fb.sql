DROP FUNCTION IF EXISTS public.adjust_correct_score_odds(uuid, numeric, numeric);

CREATE OR REPLACE FUNCTION public.adjust_correct_score_odds(p_match_id uuid, p_target_overround numeric DEFAULT 1.25, p_max_odds numeric DEFAULT 1000000.0)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  FOR r IN
    ...
    WHERE match_id = p_match_id AND market = 'correct_score' AND active = true
      AND source IS DISTINCT FROM 'api-football'
  LOOP
  ...
  UPDATE _cs SET prob = prob / v_raw_total WHERE selection IS NOT NULL;
  ...
  UPDATE public.match_market_odds m
   ...
   WHERE m.match_id = p_match_id
     ...
     AND m.source IS DISTINCT FROM 'api-football';
