-- Phase 9 — unified P/L reporting from the accounting journal ------------
...
CREATE OR REPLACE FUNCTION public.accounting_pl_report(
  ...
  p_basis text DEFAULT 'settlement',          -- 'settlement' | 'placement'
  p_products text[] DEFAULT NULL,
  p_game text DEFAULT NULL,
  p_sport text DEFAULT NULL,                  -- 'sports' | 'arcade'
...
BEGIN
  ...
  CREATE TEMP TABLE IF NOT EXISTS _pl_scope (
    journal_id uuid, product text, game text, journal_type text, attributed_at timestamptz
  ) ON COMMIT DROP;
  DELETE FROM _pl_scope;

  -- attribute every journal to a reporting date -------------------------
  INSERT INTO _pl_scope
  WITH j AS (
    SELECT x.id, x.product, x.game, x.journal_type::text AS journal_type,
           x.reference_type, x.reference_id, x.effective_at, x.metadata
  ...
  settle AS (
    SELECT reference_type, reference_id, min(effective_at) AS settled_at
      FROM j
     WHERE journal_type IN ('PAYOUT_SETTLED','REFUND','VOID')
  ...
  place AS (
    SELECT reference_type, reference_id, min(effective_at) AS placed_at
      FROM j
     WHERE journal_type = 'STAKE_PLACED' AND reference_id IS NOT NULL
     GROUP BY 1,2
  )
  SELECT j.id, j.product, j.game, j.journal_type,
         CASE
           WHEN v_basis = 'placement'
             THEN coalesce(p.placed_at, j.effective_at)
           ELSE CASE
                  WHEN j.journal_type = 'STAKE_PLACED' THEN s.settled_at
                  ELSE coalesce(s.settled_at, j.effective_at)
                END
         END
    FROM j
  ...
  -- unsettled stakes have no realised date and never count as realised P/L
  DELETE FROM _pl_scope WHERE attributed_at IS NULL
     OR attributed_at < v_from OR attributed_at > v_to;

  -- per-product realised figures ----------------------------------------
  WITH agg AS (
    SELECT s.product,
  ...
  SELECT coalesce(jsonb_agg(to_jsonb(row) ORDER BY row.grp, row.product), '[]'::jsonb)
    INTO v_products
    FROM (
      SELECT f.product,
   ...
   WHERE p_sport IS NULL OR row.grp = p_sport;
  ...
  SELECT round(coalesce(sum(l.debit - l.credit),0),2) INTO v_adjust
    FROM _pl_scope s
    JOIN public.accounting_journal_lines l ON l.journal_id = s.journal_id
    JOIN public.accounting_accounts a ON a.id = l.account_id
  ...
  RETURN jsonb_build_object(
    ...
    'platform', v_platform,
    'groups', (
      SELECT coalesce(jsonb_agg(g ORDER BY g->>'group'), '[]'::jsonb) FROM (
        SELECT jsonb_build_object(
          'group', grp,
