...
DECLARE
  ...
  v_bank_acct uuid; v_rev numeric; v_exp numeric; v_bal numeric;
BEGIN
  IF NOT public.accounting_caller_authorised() THEN RAISE EXCEPTION 'ACCOUNTING_FORBIDDEN'; END IF;

  SELECT a.user_id, b.balance INTO v_user, v_bal
  ...
  IF v_user IS NULL OR v_bal < 50 THEN
    RETURN jsonb_build_object('skipped', true, 'reason','no funded simulation wallet');
  END IF;
  ...
  FOREACH v_prod IN ARRAY ARRAY['treasure','roulette','blackjack'] LOOP
    FOR v_stake, v_payout IN SELECT * FROM (VALUES (10::numeric, 0::numeric), (10,25), (10,10), (10,10)) t(a,b) LOOP
      ...
      PERFORM public.accounting_post_arcade_settlement(v_prod,'selftest',v_ref,v_user,v_stake,v_payout,now(),
        jsonb_build_object('selftest',true), NULL, NULL);
      SELECT balance INTO v_bank_after FROM public.accounting_account_balances WHERE account_id = v_bank_acct;
      v_expect := v_stake - v_payout; v_delta := v_bank_after - v_bank_before; v_ok := (v_delta = v_expect);
      v_all_ok := v_all_ok AND v_ok;
      v_out := v_out || jsonb_build_array(jsonb_build_object('product',v_prod,'stake',v_stake,'payout',v_payout,
        'expected',v_expect,'actual',v_delta,'ok',v_ok));
      ...
      v_out := v_out || jsonb_build_array(jsonb_build_object('product',v_prod,'replay_delta',v_bank_after-v_bank_before,
        'ok', v_bank_after = v_bank_before));

      SELECT balance INTO v_bank_before FROM public.accounting_account_balances WHERE account_id = v_bank_acct;
      PERFORM public.accounting_reverse_arcade_settlement(v_prod, v_ref, 'selftest reversal check');
      SELECT balance INTO v_bank_after FROM public.accounting_account_balances WHERE account_id = v_bank_acct;
      v_ok := (v_bank_after - v_bank_before) = -v_expect; v_all_ok := v_all_ok AND v_ok;
      v_out := v_out || jsonb_build_array(jsonb_build_object('product',v_prod,'reversal_delta',v_bank_after-v_bank_before,'ok',v_ok));
    END LOOP;
  END LOOP;
