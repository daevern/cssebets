...
DECLARE
  ...
  v_pool RECORD; v_sim BOOLEAN; v_outcome TEXT;
BEGIN
  ...
  FOR v_pred IN
    ...
    WHERE match_id=p_match_id AND status='pending'::public.prediction_status
      -- New-market bets are settled by settle_new_markets_for_match
      AND COALESCE(market_text,'') NOT IN ('over_under_2_5','btts','correct_score','half_time_full_time','exact_total_goals')
    FOR UPDATE
  LOOP
    v_won := FALSE; v_payout := 0;
    -- Normalize outcome: strip any "market_key:" prefix (e.g. 'correct_score:1-0' -> '1-0')
