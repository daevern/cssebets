ALTER TABLE public.ufc_fighters
  ADD COLUMN IF NOT EXISTS age_years INTEGER,
  ADD COLUMN IF NOT EXISTS weight_lbs NUMERIC,
  ADD COLUMN IF NOT EXISTS team_name TEXT,
  ADD COLUMN IF NOT EXISTS birth_place TEXT,
  ADD COLUMN IF NOT EXISTS gender TEXT,
  ADD COLUMN IF NOT EXISTS ko_w INTEGER,
  ADD COLUMN IF NOT EXISTS ko_l INTEGER,
  ADD COLUMN IF NOT EXISTS sub_w INTEGER,
  ADD COLUMN IF NOT EXISTS sub_l INTEGER;
...
ALTER TABLE public.ufc_bets DROP CONSTRAINT IF EXISTS ufc_bets_market_type_check;
ALTER TABLE public.ufc_bets ADD CONSTRAINT ufc_bets_market_type_check
  CHECK (market_type = ANY (ARRAY[
    'moneyline'::text,
    'three_way'::text,
    'method'::text,
...
BEGIN
  ...
  FOR v_bet IN SELECT * FROM public.ufc_bets WHERE fight_id = p_fight_id AND status = 'open' FOR UPDATE LOOP
    ...
    ELSIF v_bet.market_type = 'handicap' THEN
      -- selection_key like a_plus_5_5 / b_minus_5_5. This is resolved as
      -- fight-winner handicap because the admin settlement flow records winner,
      -- method and round, not judge-by-judge scorecards.
