...
CREATE OR REPLACE FUNCTION public.adjust_correct_score_odds(
  ...
  p_max_odds numeric DEFAULT 20.0
) RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  PERFORM public.adjust_correct_score_odds(p_match_id, 1.25, 20.0);
END $function$;

-- Backfill all existing matches with correct_score odds.
DO $$
DECLARE m RECORD;
BEGIN
  FOR m IN SELECT DISTINCT match_id FROM public.match_market_odds WHERE market = 'correct_score'
  LOOP
    PERFORM public.adjust_correct_score_odds(m.match_id, 1.25, 20.0);
  END LOOP;
END $$;
