...
BEGIN
  ...
  -- Freshness anchor is kickoff time, not matches.updated_at.
  -- The previous check compared match_stats.fetched_at against matches.updated_at,
  -- which caused finished matches to be stuck as "not fresh" whenever the match
  -- row was touched (admin edit, sync, etc.) after the last stats fetch.
  ...
  -- Events considered present for a finished match if any event row exists
  -- at or after kickoff (previously gated on updated_at which caused false negatives).
  SELECT EXISTS (
    SELECT 1 FROM public.match_events
