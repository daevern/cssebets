-- 1. Update the market_odds_cap function to return 200.0 for correct_score
...
-- 2. Update the default value of p_max_odds in adjust_correct_score_odds to 200.0
CREATE OR REPLACE FUNCTION public.adjust_correct_score_odds(
  p_match_id uuid,
  p_target_overround numeric DEFAULT 1.25,
  p_max_odds numeric DEFAULT 200.0
) RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
-- 3. Update regenerate_match_market_odds to pass 200.0 instead of 100.0
CREATE OR REPLACE FUNCTION public.regenerate_match_market_odds(p_match_id uuid)
 ...
 BEGIN
   ...
   PERFORM public.adjust_correct_score_odds(p_match_id, 1.25, 200.0);
 END $function$;
...
-- 4. Re-seed and re-adjust all existing correct_score match odds under the new 200.0 cap
DO $$
DECLARE m RECORD;
BEGIN
  ...
  LOOP
    ...
    PERFORM public.adjust_correct_score_odds(m.match_id, 1.25, 200.0);
  END LOOP;
END $$;
