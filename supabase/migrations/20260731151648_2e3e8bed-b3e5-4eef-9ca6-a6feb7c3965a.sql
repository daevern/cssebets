...
BEGIN
  ...
  SELECT
    ...
    -- posted in the range but attributed elsewhere
    coalesce((SELECT round(sum(l.credit - l.debit),2)
                FROM scope s
                JOIN public.accounting_journal_lines l ON l.journal_id = s.journal_id
                JOIN public.accounting_accounts a ON a.id = l.account_id
    ...
    -- posted in the range but outside the reported journal scope
    -- (opening balances, reversed/reversal entries, filtered-out products)
    coalesce((SELECT round(sum(l.credit - l.debit),2)
