CREATE OR REPLACE FUNCTION public.place_ufc_bet_atomic(p_user_id uuid, p_fight_id uuid, p_market_type text, p_selection_key text, p_selection_label text, p_stake numeric, p_odds numeric)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Auto-create wallet row if missing (PK is user_id only)
  ...
  INSERT INTO public.wallet_transactions(user_id, type, amount, balance_before, balance_after, reference_type, reference_id, note, is_simulation, transaction_category, bet_id, metadata)
    VALUES (p_user_id, 'debit', p_stake, v_bal, v_new_bal, 'bet_placement', v_bet_id, 'UFC bet placed', false, 'ufc_bet', v_bet_id,
