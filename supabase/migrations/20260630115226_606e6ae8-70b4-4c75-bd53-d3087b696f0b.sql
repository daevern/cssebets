...
BEGIN
  ...
  -- reference_odds_snapshot_id FK references match_odds_snapshots (legacy 1X2 table),
  -- which does not store per-market snapshots. Leave NULL for non-1X2 markets to
  -- avoid FK violations; market-level history lives in market_odds_snapshots.
  v_snapshot_id := NULL;
