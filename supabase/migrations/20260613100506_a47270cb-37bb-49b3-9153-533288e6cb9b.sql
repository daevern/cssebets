-- Remove cap on correct_score: use effectively unbounded value
...
AS $$
  SELECT CASE p_market
    ...
    WHEN 'correct_score'       THEN 1000000.0
    ELSE 20.0
  END::numeric
$$;
...
-- Default p_max_odds raised to effectively unbounded
...
CREATE OR REPLACE FUNCTION public.regenerate_match_market_odds(p_match_id uuid)
 ...
 BEGIN
   ...
   PERFORM public.adjust_correct_score_odds(p_match_id, 1.25, 1000000.0);
 END $function$;
...
-- Re-seed/adjust all existing correct_score markets uncapped
DO $$
DECLARE m RECORD;
BEGIN
  ...
  LOOP
    ...
    PERFORM public.adjust_correct_score_odds(m.match_id, 1.25, 1000000.0);
  END LOOP;
END $$;
