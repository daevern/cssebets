...
function MarketCard({ match, now }: { match: Match; now: number }) {
  ...
  const pct = (() => {
    const o = match.reference_odds;
    if (!o) return null;
    const inv = { h: 1 / o.home, d: 1 / o.draw, a: 1 / o.away };
    const s = inv.h + inv.d + inv.a;
    return { home: Math.round((inv.h / s) * 100), away: Math.round((inv.a / s) * 100) };
  })();
  const isTop = live;
  ...
  return (
    ...
    >
      ...
      <div className="relative p-4">
        ...
        <div className="mt-4 flex items-center justify-center gap-2 rounded-xl bg-[var(--neon)] py-3 text-[14px] font-bold tracking-tight text-[#04140A] transition-transform group-hover:translate-y-[-1px]">
