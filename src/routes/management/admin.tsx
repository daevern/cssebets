  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900 p-4 space-y-3">
        <h2 className="font-semibold flex items-center gap-2"><ScrollText className="h-4 w-4 text-violet-300" /> Support audit log</h2>
          ...
          {(audit.data?.rows ?? []).map((r: any) => (
            <div key={r.id} className="border-b border-slate-800 py-2">
              <div className="flex items-center gap-2">
                <span className="font-mono text-violet-300">{r.action_type}</span>
