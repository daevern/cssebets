...
export const getRiskDashboard = createServerFn({ method: "GET" })
  ...
  .inputValidator((i: unknown) =>
    z.object({
      bankroll: z.number().min(0).max(100_000_000).default(50_000),
  ...
  .handler(async ({ data, context }) => {
    ...
    for (const m of matches ?? []) {
      ...
      // per-user exposure for this match
      ...
      const topUsers = [...userBuckets.values()]
      ...
      const liabilityPct = data.bankroll > 0 ? worstCaseLiability / data.bankroll : 0;
      ...
      matchRows.push({
        ...
        worstCaseLiability,
        worstOutcomeKey: worst?.key ?? null,
        bets: list.length,
        outcomes,
        topUsers,
        recommendation,
    ...
    for (const m of matchRows) {
      ...
      if (data.bankroll > 0 && m.worstCaseLiability > data.bankroll) {
        alerts.push({
          level: "critical",
          type: "bankroll_breach",
          message: `${m.match}: worst-case loss RM${m.worstCaseLiability.toFixed(0)} exceeds bankroll RM${data.bankroll}`,
          matchId: m.matchId,
        });
      }
    }

    const userThreshold = (data.userExposurePct / 100) * data.bankroll;
    ...
    if (data.bankroll > 0 && totalWorstCaseLiability > data.bankroll) {
      alerts.unshift({
        level: "critical",
        type: "total_liability",
        message: `Total worst-case liability RM${totalWorstCaseLiability.toFixed(0)} exceeds admin reserve RM${data.bankroll}`,
      });
    }
