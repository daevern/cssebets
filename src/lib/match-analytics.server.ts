// Pure server-only fetch logic for a match analytics bundle. Two thin
// server-function wrappers (auth + public) live in match-analytics.functions.ts.
import type { AnalyticsBundle } from "@/lib/match-analytics.functions";

export async function fetchMatchAnalytics(matchId: string): Promise<AnalyticsBundle> {
  ...
  const needsLineupsBackfill =
    (lineupsR.data ?? []).length === 0 &&
  ...
  const needsEventsBackfill =
    (eventsR.data ?? []).length === 0 && (phase === "live" || phase === "finished");
  const needsStatsBackfill =
    (statsR.data ?? []).length === 0 && (phase === "live" || phase === "finished");
  const needsH2HBackfill = !h2hR.data;
  const needsInjBackfill =
    (injR.data ?? []).length === 0 && (phase === "pre" || phase === "lineups");

  let l2 = lineupsR, e2 = eventsR, s2 = statsR, r2 = ratingsR, h2 = h2hR, inj2 = injR;

  if (
    needsLineupsBackfill || needsRatingsBackfill || needsEventsBackfill ||
    needsStatsBackfill || needsH2HBackfill || needsInjBackfill
  ) {
    try {
      ...
      const [nl, ne, ns, nr, nh, ni] = await Promise.all([
        needsLineupsBackfill
          ? (supabaseAdmin as any).from("match_lineups").select("*").eq("match_id", matchId)
          : Promise.resolve(lineupsR),
        needsEventsBackfill
          ? (supabaseAdmin as any).from("match_events").select("*").eq("match_id", matchId).order("minute", { ascending: true })
          : Promise.resolve(eventsR),
        needsStatsBackfill
          ? (supabaseAdmin as any).from("match_stats").select("*").eq("match_id", matchId)
          : Promise.resolve(statsR),
        needsRatingsBackfill
          ? (supabaseAdmin as any).from("match_player_ratings").select("*").eq("match_id", matchId).order("rating", { ascending: false })
          : Promise.resolve(ratingsR),
        needsH2HBackfill
          ? (async () => {
              const norm = (s: string) =>
