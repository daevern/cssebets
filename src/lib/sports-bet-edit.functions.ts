...
export const editPendingSportsBetStake = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: unknown) => EditSchema.parse(i))
  .handler(async ({ data, context }) => {
    await requireApprovedMember(context);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: result, error } = await (supabaseAdmin as any).rpc("edit_sports_bet_stake", {
...
export const cancelPendingSportsBet = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: unknown) => CancelSchema.parse(i))
  .handler(async ({ data, context }) => {
    await requireApprovedMember(context);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: result, error } = await (supabaseAdmin as any).rpc("cancel_sports_bet", {
      p_user_id: context.userId,
      p_bet_id: data.betId,
    });
    if (error) throw new Error(friendly(error.message ?? ""));
