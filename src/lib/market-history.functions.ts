...
export const getMarketHistory = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<MarketHistoryPayload> => {
    ...
    // Availability: which markets have at least one snapshot?
    ...
    if (chosen === "match_result") {
      ...
      // Group by timestamp so we can normalize (implied prob sums ≈ 100%)
      ...
      series = [...bySel.entries()]
        ...
        // Sort so most-likely selection (highest current prob) shows first in legend
        .sort((a, b) => (b.points.at(-1)?.prob ?? 0) - (a.points.at(-1)?.prob ?? 0));
      // For very wide markets (correct_score has 22 selections) trim to top 6
